...
[![版本](https://img.shields.io/badge/Release-2.3.0-38bdf8.svg)](https://github.com/YYRMMAYO/OBS_Helper/releases)
...
> **这是什么？** 面向直播新手的 OBS Studio 排障工具。**纯离线可用**：112 条问题的知识库、排障指引、日志分析规则全部内嵌在程序里，不联网也能查。连上 OBS 之后还能远程控制场景、录制与推流，并做一键体检。
>
...
>
> **V2.2 起内置插件广场**：38 个精选插件收录、本机已装插件只读体检（全盘定位 OBS 安装目录）、日志分析联动插件嫌疑跳转、直达 Releases 下载；**V2.1 起支持增量更新与知识库独立更新**：更新只下载变更文件，问题库与插件目录都可脱离应用单独升级。
...
| | |
|---|---|
| **112 条问题库，完全离线** | 内置 **10 个分类、112 条问题**（v1.6），含现象 / 成因 / 分步解决 / 小贴士 / 相关问题；步骤可勾选，进度自动记住。**知识库支持独立更新**（应用之外随拉随新，不用等发版） |
| **插件广场 + 本机体检** | 收录 **38 个精选插件**（7 分类，链接逐一验证），每张卡片直达 GitHub Releases 下载；**只读扫描本机已装插件**——全盘定位 OBS 安装目录（注册表多视图 / 全盘驱动器布局 / Steam 多库），装在非 C 盘也不漏扫；日志分析发现可疑模块可一键跳转对应插件 |
| **增量更新，更新更轻** | V2.1 起应用内「增量更新」只下载变更文件（如 2.2 → 2.3.0 增量包约 1.3MB）；逐文件 SHA-256 校验，失败自动回退完整安装包；安装版自动提权替换并重启 |
...
| **深度日志分析** | 离线解析 OBS 日志：**23 条规则 + 3 项量化比值**；日志在分析前**先脱敏**；掉帧 / 崩溃线索可关联到嫌疑插件并直达插件广场 |
...
- **知识库** — 10 个分类、112 条问题（v1.6），含现象 / 成因 / 分步解决 / 小贴士 / 相关问题；步骤可勾选且进度会记住
...
- **精选插件目录** — 7 大分类 × 38 个精选插件（自动化 / 视觉特效 / AI 智能 / 音频 / 多平台竖屏 / 录制后期 / 输入互动），目录外置热更新，链接纠错与新插件上架无需发版
...
> 已安装旧版（2.1.x / 2.2.x）的用户可直接走应用内「增量更新」升级到 2.3.0。
...
- `OBS_Helper_Setup_2.3.0.exe` — 安装包
- `OBS_Helper_Portable_2.3.0.zip` — 解压即用
- `OBS_Helper_Update_2.3.0.zip` — 增量更新包（含 `update_manifest.json`，应用内增量更新用）
- `OBS_Helper_Manifest_2.3.0.json` — 完整文件清单（SHA-256，比对/核验用）
- `OBS_Helper_Portable_2.3.0.exe` — 单文件（需 `-SingleFile`）
