...
- **迷你小窗** — 置顶小窗一键开始 / 停止录制与推流（含虚拟摄像头），可拖拽、记住位置；托盘菜单或全局热键（默认 Ctrl+Alt+M）随时呼出
...
- `OBS_Helper_Setup_1.6.0.exe` — 安装包
- `OBS_Helper_Portable_1.6.0.zip` — 解压即用
- `OBS_Helper_Portable_1.6.0.exe` — 单文件（需 `-SingleFile`）
