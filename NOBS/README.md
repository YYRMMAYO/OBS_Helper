...
- **智能诊断** — 连上 OBS 后一键体检；三种引擎可选：本地规则引擎（默认，离线零成本）、**免费 AI（内置，无需注册与 API Key，本地强制每日限次，适合低频使用）**、云端大模型（OpenAI 兼容接口 + function calling，接入自己的 API Key）；云端/免费失败自动回退本地；**结果可导出为 Markdown 报告**
...
内置免费 AI，**开箱即用**，两条通道可选、**各自独立限频**：`智谱免费模型`（国内直连最稳；密钥在构建时由 `scripts/embed_free_ai_key.ps1` 加密内嵌进安装包，多重加密 + 不入 git 仓库，详见 `docs/` 审查报告）与 `Pollinations`（国外免 Key 公共通道，无需任何密钥，适合能直连国际网络的用户）。为了不给免费服务加压，**免费档由应用本地强制限频**（V1.8.1 起按通道分开计数）：

- **智谱通道**：每台机器每天最多 **10 次** 诊断（强限制档，每次发起计 1 次，失败重试也计数）；
- **Pollinations 通道**：每台机器每天最多 **20 次** 诊断（维持原低频档不变）；
- 两通道各自独立计数、互不挤占额度；计数保存在本机 `prefs.json`，跨重启生效，每天 0 点自动恢复；
...
- `OBS_Helper_Setup_1.8.1.exe` — 安装包
- `OBS_Helper_Portable_1.8.1.zip` — 解压即用
- `OBS_Helper_Portable_1.8.1.exe` — 单文件（需 `-SingleFile`）
